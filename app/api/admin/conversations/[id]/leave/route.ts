...
// POST /api/admin/conversations/[id]/leave - Admin leaves a conversation
export async function POST(
  ...
  try {
    ...
    const body: LeaveConversationRequest = await request.json();
    ...
    // Check if admin is the current admin in conversation
    if (conversation.admin_id !== body.admin_id) {
      return NextResponse.json(
        { error: 'Admin is not currently in this conversation' },
        { status: 403 }
      );
    }

    // Update conversation to remove admin_id
    ...
    console.error('Admin leave conversation error:', error);
